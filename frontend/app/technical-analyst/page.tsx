  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col lg:flex-row gap-4">
        ...
        <aside className="lg:w-1/4 lg:min-w-[240px] rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0e1015]/70 backdrop-blur-md shadow-sm dark:shadow-xl overflow-hidden flex flex-col max-h-[45vh] lg:max-h-[calc(100vh-11rem)] transition-colors duration-300">
          <div className="p-3 border-b border-slate-200 dark:border-white/10">
            <div className="flex items-center gap-2 rounded-lg border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/[0.03] px-2.5 py-1.5">
              <Search className="w-3.5 h-3.5 text-slate-400 dark:text-zinc-500 shrink-0" />
              <input
                value={query}
                onChange={(e) => setQuery(e.target.value)}
                placeholder="Cari aset…"
                className="w-full bg-transparent text-sm text-slate-800 dark:text-zinc-200 placeholder:text-slate-400 dark:placeholder:text-zinc-600 focus:outline-none"
              />
            </div>
          </div>

          <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-zinc-700 scrollbar-track-transparent">
            {rows.length === 0 ? (
              <p className="p-4 text-xs text-slate-400 dark:text-zinc-500">Tidak ada aset cocok.</p>
            ) : (
              rows.map((r) => {
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      isActive
                        ...
                        : "border-transparent hover:bg-gradient-to-r hover:from-emerald-50 hover:to-transparent dark:hover:from-emerald-900/20 dark:hover:to-transparent",
                    )}
                  >
                    <div className="min-w-0">
                      ...
                      >
                        {r.coin}
                        <span className="ml-1 text-[10px] font-medium text-slate-400 dark:text-zinc-600">/ USDT</span>
                      </p>
                      <p className="text-[11px] font-mono text-slate-500 dark:text-zinc-500">{fmtPrice(r.price)}</p>
                    </div>
                    {typeof r.change_24h === "number" ? (
                      ...
                      <span className="text-[11px] text-slate-400 dark:text-zinc-600 shrink-0">—</span>
                    )}
                  </button>
                );
              })
        ...
        <section className="lg:flex-1 rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#0A0A0A] overflow-hidden shadow-sm dark:shadow-xl transition-colors duration-300">
